package com.jiahe.intellipichub.manager.auth;
...
import static com.jiahe.intellipichub.constant.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    /**
     * 返回一个账号所拥有的角色标识集合 (权限与角色可分开校验),本项目用不到
     */
    @Override
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        // 获取请求参数;判断contentType是否是json类型l;post请求；为了解决：在 config 包下自定义请求包装类和请求包装类过滤器。
        ...
        if (ObjUtil.isNotNull(id)) {
            // 获取到请求路径的业务前缀；request.getRequestURI() 会返回不包含域名、参数的 URI 路径部分
            String requestUri = request.getRequestURI();
            // 先替换掉上下文，剩下的就是前缀‘/api/picture/aaa?a=2'
            String partUri = requestUri.replace(contextPath + "/", "");
            // 获取前缀的，第一个斜杠前的字符串，用于判断类别
